...
impl Reg {
    ...
    fn derivative(&self, ch: char) -> Reg {
        match self.kind() {
            ...
            RegKind::Concat(parts) => {
                // ∂(r₁r₂…rₙ) = ∂r₁ · r₂…rₙ  ∪  (if ν(r₁)) ∂(r₂…rₙ)  ∪ …
                let mut alternatives = Vec::new();
                for i in 0..parts.len() {
                    let head = parts[i].derivative(ch);
                    let mut tail: Vec<Reg> = vec![head];
                    tail.extend(parts[i + 1..].iter().cloned());
                    alternatives.push(Self::concat(tail));
                    if !parts[i].nullable() {
                        break;
                    }
                }
            ...
            RegKind::Alt(parts) => Self::alt(parts.iter().map(|p| p.derivative(ch)).collect()),
            RegKind::Star(inner) => {
                // ∂(r*) = ∂r · r*
                Self::concat(vec![inner.derivative(ch), self.clone()])
            }
        }
    }
}
    ...
    while let Some(node_id) = queue.pop_front() {
        ...
        for ch in reps {
            ...
            let next_left = left_cache
                ...
                .or_insert_with(|| nodes[node_id].key.left.derivative(ch))
                .clone();
            let next_right = right_cache
                .entry((nodes[node_id].key.right.clone(), ch))
                .or_insert_with(|| nodes[node_id].key.right.derivative(ch))
                .clone();
            let next = ProductKey {
                left: next_left,
                right: next_right,
    ...
    while let Some(node_id) = queue.pop_front() {
        ...
        for ch in representative_chars(&first) {
            ...
            let next_reg = cache
                ...
                .or_insert_with(|| nodes[node_id].key.0.derivative(ch))
                .clone();
...
fn match_brzozowski(expr: &Expr, input: &str, config: &Config) -> BackendResult {
    ...
    for ch in input.chars() {
        ...
        let next = match transitions.get(&(current, ch)) {
            Some(&id) => id,
            None => {
                let next_reg = interner.states[current].derivative(ch);
...
mod tests {
    ...
    fn derivative_of_literal() {
        ...
        assert_eq!(r.derivative('a'), Reg::eps());
        assert_eq!(r.derivative('b'), Reg::null());
    }
